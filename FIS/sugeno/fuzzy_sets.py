...
class Pi_MF(MF_object):
    ...
    def _execute(self, x):
        mid_ab = (self._a + self._b) / 2.0
        mid_cd = (self._c + self._d) / 2.0
        if x <= self._a:
            return 0.0
        elif self._a <= x <= mid_ab:
            return 2 * np.power((x - self._a) / (self._b - self._a), 2)
        elif mid_ab <= x <= self._b:
            return 1 - 2 * np.power((x - self._b) / (self._b - self._a), 2)
        elif self._b <= x <= self._c:
            return 1.0
        elif self._c <= x <= mid_cd:
            return 1 - 2 * np.power((x - self._c) / (self._d - self._c), 2)
        elif mid_cd <= x <= self._d:
            return 2 * np.power((x - self._d) / (self._d - self._c), 2)
        else:
            return 0.0


class Z_MF(MF_object):
    ...
    def _execute(self, x):
        ...
        if x <= self._a:
            ...
            return 1 - 2 * np.power((x - self._a) / (self._b - self._a), 2)
        elif mid <= x <= self._b:
            return 2 * np.power((x - self._b) / (self._b - self._a), 2)
        else:
            return 0.0


class S_MF(MF_object):
...
class FuzzySet(object):
    ...
    def __init__(self, points=None, function=None, term="", high_quality_interpolate=False, boundary_values=None,
                 verbose=False):
        self._term = term
...
class PiFuzzySet(FuzzySet):
    def __init__(self, a,b,c,d, term):
        pi_MF = Pi_MF(a,b,c,d)
        super().__init__(function=pi_MF, term=term)
...
class ZFuzzySet(FuzzySet):
    def __init__(self, a, b, term):
        z_MF = Z_MF(a,b)
        super().__init__(function=z_MF, term=term)
...
class SFuzzySet(FuzzySet):
    def __init__(self, a, b, term):
        s_MF = S_MF(a, b)
        super().__init__(function=s_MF, term=term)
