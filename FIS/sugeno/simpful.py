...
class LinguisticVariable(object):
    ...
    def __repr__(self):
        ...
        str(self._FSlist), str(self._universe_of_discourse))
...
class FuzzySystem(object):
    ...
    def __init__(self, operators=None, show_banner=True, sanitize_input=False, verbose=True):
        ...
        if sanitize_input and verbose:
            print(
                " * Warning: Simpful rules sanitization is enabled, please pay attention to possible collisions of symbols.")
            ...
            try:
                if den == 0.0:
                    ...
                    if not ignore_warnings:
                        print(
                            "WARNING: the sum of rules' firing for variable '%s' is equal to 0. The result of the Sugeno inference was set to 0." % output)
                else:
    ...
    def mediate_Mamdani(self, outputs, antecedent, results, ignore_errors=False, ignore_warnings=False, verbose=False,
                        subdivisions=1000):
    ...
    def Mamdani_inference(self, terms=None, subdivisions=1000, ignore_errors=False, ignore_warnings=False,
                          verbose=False):
        """
        ...
        array_rules = array(self._rules, dtype=object)
        if len(self._constants) == 0:
            result = self.mediate_Mamdani(terms, array_rules.T[0], array_rules.T[1], ignore_errors=ignore_errors,
                                          ignore_warnings=ignore_warnings, verbose=verbose, subdivisions=subdivisions)
        else:
            # remove constant variables from list of variables to infer
            ncost_terms = [t for t in terms if t not in self._constants]
            result = self.mediate_Mamdani(ncost_terms, array_rules.T[0], array_rules.T[1], ignore_errors=ignore_errors,
                                          ignore_warnings=ignore_warnings, verbose=verbose, subdivisions=subdivisions)
            # add values of constant variables
            cost_terms = [t for t in terms if t in self._constants]
            for name in cost_terms:
    ...
    def inference(self, terms=None, subdivisions=1000, ignore_errors=False, ignore_warnings=False, verbose=False):
        ...
        if self._detected_type == "Sugeno":
            return self.Sugeno_inference(terms=terms, ignore_errors=ignore_errors, ignore_warnings=ignore_warnings,
                                         verbose=verbose)
        elif self._detected_type == "probabilistic":
            return self.probabilistic_inference(terms=terms, ignore_errors=ignore_errors,
                                                ignore_warnings=ignore_warnings, verbose=verbose)
        elif self._detected_type is None:  # default
            return self.Mamdani_inference(terms=terms, subdivisions=subdivisions, ignore_errors=ignore_errors,
                                          ignore_warnings=ignore_warnings, verbose=verbose)
        else:
            raise Exception(
                "ERROR: simpful could not detect the model type, please use either Sugeno_inference() or Mamdani_inference() methods.")
